...
| 3 | 初始化与启动 | devmind init/start/status正常 | 守护进程启动失败、端口冲突(如有) |
...
基础配置步骤包括:在MCP客户端添加DevMind服务器;可选初始化CLI;启动监控守护进程;验证运行状态[^1]。
...
- CLI初始化与启动:执行 `devmind init`;执行 `devmind start`;通过 `devmind status` 验证状态[^1]。
...
| 步骤 | 命令 | 预期输出 | 排查点 |
|---|---|---|---|
| 启动 | devmind start | 守护进程启动日志 | 权限、端口占用(如有) |
| 状态 | devmind status | running或active状态 | 进程未启动、配置错误 |
...
## 4. CLI命令参考(完整命令行使用说明和示例)

CLI围绕四类任务展开:快速开始、搜索查询、维护操作、备份恢复。所有命令均提供明确参数与示例,便于在脚本与CI中复用[^1]。
